...
try {
	$sql = "SELECT *FROM shipmentregister where  originport= '$_POST[originport]'  and branch ='$_SESSION[branch]' and fy='$_SESSION[fy]'";
	$res = $pdo->query($sql); 
	if ($res->rowCount() > 0) { 
		echo "<table id='sr'>"; 
		echo "<tr>";
...
$conn = new mysqli('localhost', 'root', '', 'importair') ;
 $result = $conn->query("SELECT *FROM shipmentregister where originport= '$_POST[originport]'  and branch ='$_SESSION[branch]' and fy='$_SESSION[fy]'");
