...
try {
	$sql = "SELECT *FROM eaediitem where  jobno='$_SESSION[jobno]' and branch ='$_SESSION[branch]' and fy='$_SESSION[fy]'";
	$res = $pdo->query($sql); 
	if ($res->rowCount() > 0) { 
		echo "<table id='esi'>"; 
...
unset($pdo); 
?>			
<button onclick="exportTableToExcel('esi','eaediitemdet')">Export Table Data To Excel File</button> 
	</body>
</head>
</html>	