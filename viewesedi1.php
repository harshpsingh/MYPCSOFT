...
try {
	$sql = "SELECT *FROM edisb1 where  exportername='$_POST[exportername]' and branch ='$_SESSION[branch]' and fy='$_SESSION[fy]'";
	$res = $pdo->query($sql); 
	if ($res->rowCount() > 0) { 
		echo "<table id='sr'>"; 
		echo "<tr>";
...
$conn = new mysqli('localhost', 'root', '', 'exportsea') ;
 $result = $conn->query("SELECT *FROM edisb1 where  exportername='$_POST[exportername]' and branch ='$_SESSION[branch]' and fy='$_SESSION[fy]'");
