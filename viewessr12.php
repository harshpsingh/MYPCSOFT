...
try {
	$sql = "SELECT *FROM shipmentregister where dischargecountry= '$_POST[dischargecountry]'  and branch ='$_SESSION[branch]' and fy='$_SESSION[fy]'";
	$res = $pdo->query($sql); 
	if ($res->rowCount() > 0) { 
		echo "<table id='sr'>"; 
		echo "<tr>";
...
$conn = new mysqli('localhost', 'root', '', 'exportsea') ;
 $result = $conn->query("SELECT *FROM shipmentregister where  dischargecountry ='$_POST[dischargecountry]' and branch ='$_SESSION[branch]'");
