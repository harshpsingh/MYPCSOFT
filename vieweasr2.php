...
try {
	$sql = "SELECT *FROM shipmentregister where  exportername= '$_POST[exportername]'  and branch ='$_SESSION[branch]' and fy='$_SESSION[fy]'";
	$res = $pdo->query($sql); 
	if ($res->rowCount() > 0) { 
		echo "<table id='sr'>"; 
		echo "<tr>";
...
$conn = new mysqli('localhost', 'root', '', 'exportair') ;
 $result = $conn->query("SELECT *FROM shipmentregister exportername= '$_POST[exportername]'  and branch ='$_SESSION[branch]' and fy='$_SESSION[fy]'");
