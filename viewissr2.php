...
try {
	$sql = "SELECT *FROM shipmentregister where  importername= '$_POST[importername]'  and branch ='$_SESSION[branch]' and fy='$_SESSION[fy]'";
	$res = $pdo->query($sql); 
	if ($res->rowCount() > 0) { 
		...
		echo "<th>terminal</th>";
		echo"<th>Firstvessel</th>";
		echo"<th>Secondvessel</th>";
		echo "<th>Voyage</th>";
		echo "<th>eta</th>";
...
$conn = new mysqli('localhost', 'root', '', 'importsea') ;
 $result = $conn->query("SELECT *FROM shipmentregister where  importername ='$_POST[importername]' and branch ='$_SESSION[branch]'");
